...
if ($existing) {
    if ($existing.State.ToString() -ne 'Disabled' -or $existing.Actions.Count -ne 1 -or
        $existing.Actions[0].Execute -ne $python -or $existing.Actions[0].Arguments -ne '-m src.history_supervisor' -or
...
$action = New-ScheduledTaskAction -Execute $python -Argument '-m src.history_supervisor' -WorkingDirectory $root
...
if ($Start) { Start-ScheduledTask -TaskName $taskName }
Write-Output 'History supervisor installed: single child, bounded retries, persistent safety circuit'
